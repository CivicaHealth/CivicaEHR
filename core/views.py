...
def referral_list(request):
    ...
    counts = {
        ...
        "sent":      Referral.objects.filter(status="sent").count(),
...
@login_required
@role_required(STUDENT, ATTENDING)
def referral_create(request, patient_id):
    patient = get_patient_for_user(request.user, patient_id)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            referral.save()
            log_action(request=request, action="create", instance=referral)
...
@login_required
@role_required(STUDENT, ATTENDING)
def referral_edit(request, referral_id):
    referral = get_owned_object(Referral, request.user, id=referral_id)

    if request.method == "POST":
        form = ReferralForm(request.POST, instance=referral)
        if form.is_valid():
            form.save()
            log_action(request=request, action="update", instance=referral)
